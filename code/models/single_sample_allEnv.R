model{
  ...
  # Data model ----
  for(i in 1:length(d11BGrub.obs)){
    d11BGrub.obs[i] ~ dnorm(d11Bf[d11BGrub.ai[i]], d11BGrub.pre[i])
    d11BGrub.pre[i] = 1 / d11BGrub.se[i] ^ 2
  }

  for(i in 1:length(d11BTsac.obs)){
    d11BTsac.obs[i] ~ dnorm(d11Bf[d11BTsac.ai[i]], d11BTsac.pre[i])
    d11BTsac.pre[i] = 1 / d11BTsac.se[i] ^ 2
  }
  
  for(i in 1:length(d18Of.obs)){
    d18Of.obs[i] ~ dnorm(d18Of[d18Of.ai[i]], d18Of.pre[i])
    d18Of.pre[i] = 1 / d18Of.se[i] ^ 2
  }
  
  for(i in 1:length(mgcaf.obs)){
    mgcaf.obs[i] ~ dnorm(mgcaf[mgcaf.ai[i]], mgcaf.pre[i])
    mgcaf.pre[i] = 1 / mgcaf.se[i] ^ 2
  }
  
  for(i in 1:length(d13Cc.obs)){
    d13Cc.obs[i] ~ dnorm(d13Cc[d13Cc.ai[i]], d13Cc.pre[i])
    d13Cc.pre[i] = 1 / d13Cc.se[i] ^ 2
  }
  
  for(i in 1:length(d18Oc.obs)){
    d18Oc.obs[i] ~ dnorm(d18Oc[d18Oc.ai[i]], d18Oc.pre[i])
    d18Oc.pre[i] = 1 / d18Oc.se[i] ^ 2
  }
  
  for(i in 1:length(D47c.obs)){
    D47c.obs[i] ~ dnorm(D47c[D47c.ai[i]], D47c.pre[i])
    D47c.pre[i] = 1 / D47c.se[i] ^ 2
  }
